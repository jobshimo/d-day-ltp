...
@Component({
  ...
  template: `
    <svg>
      <ddob-unit-symbol [type]="type" [germanSymbol]="germanSymbol" [color]="color" />
    </svg>
  `,
})
class HostComponent {
...
describe('UnitSymbolComponent — US types', () => {
  it('infantry renders two diagonal <line> elements (crossed diagonals)', () => {
    ...
    const lines = el.querySelectorAll('ddob-unit-symbol line');
  ...
  it('tank renders an <ellipse> element (armor oval)', () => {
    ...
    const ellipse = el.querySelector('ddob-unit-symbol ellipse');
    expect(ellipse).toBeTruthy();
  });

  it('arty renders a filled <circle> (artillery dot)', () => {
    const el = setupUS('arty');
    const circle = el.querySelector('ddob-unit-symbol circle');
  ...
  it('general renders a <polygon> (star, no box rect)', () => {
    ...
    const polygon = el.querySelector('ddob-unit-symbol polygon');
    expect(polygon).toBeTruthy();
    // General has no box
    const box = el.querySelector('ddob-unit-symbol rect');
    expect(box).toBeNull();
  });

  it('engineer renders a <polyline> bracket shape', () => {
    const el = setupUS('engineer');
    const polyline = el.querySelector('ddob-unit-symbol polyline');
  ...
  it('color input is applied to stroke attributes', () => {
    ...
    const line = el.querySelector('ddob-unit-symbol line');
...
describe('UnitSymbolComponent — German types', () => {
  ...
  it('german artillery-88 renders a <text> element containing "88"', () => {
    ...
    const text = el.querySelector('ddob-unit-symbol text');
